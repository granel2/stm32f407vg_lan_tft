...
**J2** +12V вход (правый верх), **J3** CAN1 4-pin (левый низ: GND, CANL, CANH, +5V), D4 PESD1CAN, R2/R30 2×49.9 Ω — split-терминатор 120 Ω впаян постоянно (узел должен быть на конце шины). Драйвер в прошивке — [`../CAN/`](../CAN/README.md) (CAN1, PA11/PA12).
...
- DC/RST/BL/кнопка страниц — четыре соседних пина SV1 (PD6/PD5/PC7/PC6, SV1.10/SV1.9/SV1.8/
  SV1.7). SV5 в разводке TFT не используется. PC6/PC7/PD5/PD6 здесь подписаны как
  USART6/USART2 TX-RX, но в прошивке не заняты (используются только USART1 и USART3).
- VCC дисплея: **+5V с SV4.5** — на модуле есть свой стабилизатор (U8, SOT-23-3, маркировка
  6206A = Torex XC6206P332MR или клон, фикс. 3.3 В, подтверждено визуально); если бы модуль
  был чисто 3.3-вольтовый — брать +3V3 с SV2.1, но в данном случае это не требуется.
  Логика ST7796S 3.3 V, уровни от STM32 подходят.
